...
**The loop trips the gate on its own account, not only through what it contains.** An observed prompt gave its reason as `Contains for_statement` — a check on the statement type, which fires whether or not the body carries an expansion. So `simple_expansion` names one reason among several, and stripping the expansion out of a loop does not make the loop allowlistable. `tool-and-shell-safety.md`'s batching rule carries the practical form and the rest of the parser-node family.
