...
- When it isn't yet clear who the file is for, start it under `.claude/scratch/` anyway. The bullet above assumes you know at writing time that the artifact is for the user, and often you don't: a script written for your own use turns out to be the evidence behind a decision, a scratch query turns out to be worth re-running next session. Rescuing a file *out* of the session scratchpad is a chore that has to be remembered, and it mostly isn't. Deleting a directory the user can see is a chore that can be skipped at no cost, since `.claude/` is gitignored and nothing there can reach the repo. So invert the default — only genuinely one-shot output stays in the session scratchpad, and anything you might plausibly want to look at twice starts under `.claude/scratch/`. Keep it in that subdirectory rather than loose in `.claude/`, which projects often document as a curated layout, and clear of `.claude/handoffs/`, which handoff tooling scans. Failure mode: a file that only became worth keeping *after* it was written evaporates with the session, or survives only because the model happened to remember to offer moving it.
