...
- When referencing a commit in a durable artifact (handoff, PR description, ADR, issue, session doc, another commit message), prefer the commit *message* or a branch-plus-position reference over the bare SHA — unless the commit is on `main` or otherwise in merged, immutable history. SHAs are precise and are what `git log --oneline` surfaces, so they're the reflexive choice. But a SHA on any unmerged commit is unstable — rebase, amend, and squash-on-merge all rewrite it, leaving the reference pointing at nothing. Rebasing feature branches freely (the user's default) makes this bite often, but it isn't the only cause. Quote the subject line ("docs: add ADR 0011 …") or describe position ("the ADR 0011 commit", "the tip of `ym/graphify`") instead. If a SHA is genuinely useful, pair it with its subject so the reference still resolves by meaning after the hash is gone. Failure mode: a handoff or PR body names `32ed925`, the branch is rebased onto a moved `main`, and the SHA now resolves to nothing — the reader cannot find what was referenced. Sibling of the file-tracked-before-referencing rule above. Same root — references in durable artifacts must survive the machine and the history they were written against.
