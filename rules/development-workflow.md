...
- When feature work is in flight and dependabot PRs are open, land your own branches to `main` *before* merging the dependabot PRs. Dependabot's default `rebase-strategy: auto` rebases its open PRs onto the advanced base and regenerates the lockfile itself, so letting your work land first pushes the `Gemfile.lock` conflict-resolution onto the bot. The reverse order — merging dependabot first, then updating your branch onto the bumped `main` — makes *you* hand-resolve the lockfile, and a rebase re-hits the conflict once per lockfile-touching commit (a merge resolves it once, which is the better tool if this order can't be avoided). The clean lockfile resolution is not to hand-merge conflict markers but to take one side as the base and regenerate with `bundle` — watch that `bundle` doesn't silently revert a deliberate `BUNDLED WITH` pin to the other side's value. Failure mode: a routine dependency bump merged ahead of in-flight work turns a clean branch update into a repeated manual `Gemfile.lock` merge.
