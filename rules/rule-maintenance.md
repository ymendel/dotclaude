...
**Where a session in either config tree is reachable, message it rather than commit.** An out-of-repo edit can hand its context to a session already sitting in the tree, instead of committing from outside — which invites an amend during the refinement pass — or loading the rule prose with provenance that belongs in a commit body. **The receiving session owns that commit**, including the parts a peer wrote. Ask what a session can **reach** rather than which repo it sits in: those come apart, and a session's name is evidence of neither. A note is different and goes straight in, per `cross-project-notes.md`.
