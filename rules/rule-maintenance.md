...
## Disambiguate scope and tree before editing
...
**`dotfiles` is a different tree with a different job, and the overlap resolves here.** The machine-setup repo and this one both read as "configuration", which is what makes them confusable, but anything that comes up in Claude belongs in `dotclaude` entirely. Where a concern genuinely spans both — a language style preference this config states and a linter config in `dotfiles` enforces — it is still *primarily* here, so draft it here and let the other side follow. Never route a rule, skill, hook, or settings change to `dotfiles` on the grounds that it is config-flavoured. Failure mode: a Claude-config change is aimed at the machine-setup repo because the two are adjacent in the user's mind and in their directory listing, and it lands somewhere nothing loads it from.
...
**Where a session in either config tree is reachable, message it rather than commit.** Peer sessions are listable with `ListAgents` and addressable with `SendMessage`, so an out-of-repo edit can hand its context to a session already sitting in the config tree — what the edit is for, the incident behind it, which parts were left rough. That avoids the two options such an edit otherwise picks between. One is committing the change from outside, which invites an amend as soon as the wording gets reworked in the refinement pass. The other is loading the rule prose with the dated incident and the provenance that belong in a commit body, per *Where dated observations go* below — a message carries both without either landing in an always-loaded file. Read *either tree* literally: a private rule reached through the `rules/private` symlink is a rule edit like any other, so the public repo's name is not what decides this. A listed name is not evidence of which tree a session sits in. Names arrive from several documented sources — a naming model working from conversation or plan content, an explicit `/rename`, a hostname prefix, a uniquifying variant when two live sessions collide — so a name that merely reads as configuration says nothing about the repo behind it, per *`dotfiles` is a different tree* above. Confirm rather than infer, and ask what a session can **reach** rather than which repo it sits in. Those come apart: a session whose working directory is the public tree can still commit to the private one when that tree is an added working directory, since `git -C <path>` needs reachability rather than residence. Asking the residence question gets a true answer that routes the edit wrongly, which is worse than no answer — the session says "I'm in the public repo", the handover goes elsewhere, and the one session that could have landed it has just talked itself out of doing so. Often no such session is running at all; when none is, report what changed and leave it, exactly as above. The observed correspondences between names and directories, and the changelog entries showing the other naming paths that make them unsafe to rely on, are in the cross-session-messaging note.
...
**The message goes out with the edit, not when somebody asks for it.** Nothing prompts it otherwise. The editing session's work is finished, the user is the audience visibly in front of it, so the report goes to them — and the session in the tree never learns the edit exists. Waiting to be asked also throws away the reason for messaging at all, since the context that made the edit obvious is what a later prompt cannot reconstruct. Send it when the edit lands, in the same turn.

**The session in the tree owns the commit, and claims it rather than handing it back.** The paragraph above tells the editing session not to commit, which leaves the commit unowned unless this is said: the receiving session commits, including the parts a peer wrote. Never answer a handover by inviting the other session to commit after all — both sides then defer to the other and the change sits dirty indefinitely, which is worse than either having just committed it.

Two things the receiver owes. Read what the prose *asserts* before staging it, per `honesty.md`'s *Check the Claims in Prose You Are About to Commit* — a style pass is not a review, and committing puts your name on claims you neither made nor checked. And where the file also carries a concurrent session's edit, commit the whole file and name the other change in the body rather than handing the commit away: `git add --patch` is interactive and unavailable here, so there is no way to stage one hunk, and that constraint is a reason to write a clearer commit body rather than to give up the commit. Where the peer's content is not ready to land, ask it to hold rather than waiting silently.

Failure mode this prevents: the division of labour reads as obvious from each side and is agreed by neither, so an edit either sits uncommitted while both sessions wait, or lands from outside the tree — the exact thing the messaging paragraph exists to avoid. Nothing errors, and each session's own conduct looks correct in isolation.
