...
Failure mode this prevents: an extra agent looks free, because the resource it consumes is invisible
from inside the session — so the user finds out at the weekly limit, having never been asked.
...
Failure mode this prevents: long sessions accumulate context from noisy reads that were only useful for extracting one or two facts, and by the time the important work arrives the window is full of incidental output.
...
Once a skill has fired in-session its content cannot be delegated anywhere. An agent whose `skills:` frontmatter names the skill closes that gap, preloading the skill's full content into a context that ends when the agent returns — `mermaid-diagram-specialist` against a 124K package is the standing instance. **Reach for a wrapper only where the package is large enough that isolation is the point**, and note that reference-material and task-shaped skills want opposite arrangements: the wrong one returns nothing.

`rules/references/agents/skill-wrapping.md` carries both arrangements, the `context: fork` alternative, and why no frontmatter field can enforce the ordering a wrapper depends on. Load it before authoring or fixing a wrapper agent.

Failure mode this prevents: the wrapper exists, is correct, and never runs, because the skill it wraps fires on description-match before any delegation decision is made. The library loads into the main context anyway, and the agent looks like configuration that was never needed rather than configuration that keeps being bypassed.
