...
When you reference a file, write it as a single literal path a reader can act on directly. For a file under the session's project root, prefer a repo-relative path (`skills/adr/SKILL.md`) — the resuming agent starts at the root, so it resolves there and reads without the noise of a full home-rooted path. Reserve an absolute path for a reference with no project root to anchor to: a sibling `--add-dir` repo, a home-dir file, a `/tmp` artifact. A `~` shorthand reads nicely, but Step 3's referenced-file check joins the path literally rather than expanding `~`, so `~/…` references always report unverified — give the full absolute path when you want a home-dir reference to resolve. Avoid any compressed or abbreviated form: a `...` gap mid-path, brace expansion (`path/to/{a,b,c}.md`), a glob (`references/*.md`), or a count-plus-pattern stand-in for a list of files. A resuming agent acts on each reference literally — opening it, `cat`-ing it, following it — and cannot act on a form it would first have to expand. Compressed forms also trip Step 3's referenced-file check, which resolves only literal paths, and getting used to waving those warnings away as "just shorthand" erodes the check's ability to catch a genuinely dangling reference. If several files are worth naming, list them.
