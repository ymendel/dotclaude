...
When you reference a file, write its full path — ideally absolute — as a single literal path a reader can act on directly. Avoid any compressed or abbreviated form: a `...` gap mid-path, brace expansion (`path/to/{a,b,c}.md`), a glob (`references/*.md`), or a count-plus-pattern stand-in for a list of files. A resuming agent acts on each reference literally — opening it, `cat`-ing it, following it — and cannot act on a form it would first have to expand. Compressed forms also trip Step 3's referenced-file check, which resolves only literal paths, and getting used to waving those warnings away as "just shorthand" erodes the check's ability to catch a genuinely dangling reference. If several files are worth naming, list them.
