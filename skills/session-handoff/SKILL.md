...
When you reference a file, write it as a single literal path a reader can act on directly. For a file under the session's project root, prefer a repo-relative path (`skills/adr/SKILL.md`) — the resuming agent starts at the root, so it resolves there and reads without the noise of a full home-rooted path. Reserve an absolute path for a reference with no project root to anchor to: a sibling `--add-dir` repo, a home-dir file, a `/tmp` artifact. For the home-dir case, a `~` shorthand is the cleaner spelling — Step 3's referenced-file check expands `~` before resolving, so `~/notes/foo.md` verifies just like the full path. Avoid any compressed or abbreviated form: a `...` gap mid-path, brace expansion (`path/to/{a,b,c}.md`), a glob (`references/*.md`), or a count-plus-pattern stand-in for a list of files. A resuming agent acts on each reference literally — opening it, `cat`-ing it, following it. A leading `~` is fine — it resolves to one fixed path. What the agent cannot act on is a form that stands in for several files, or a path with a gap it would have to reconstruct. Compressed forms also trip Step 3's referenced-file check, which resolves single concrete paths, not patterns, and getting used to waving those warnings away as "just shorthand" erodes the check's ability to catch a genuinely dangling reference. If several files are worth naming, list them.

**Re-verify anything you carry forward from a prior handoff, or mark it as unverified.** A pending item that was not acted on this session is the one thing in the new document nobody checked. It gets copied across because it is still open, it lands in "Immediate Next Steps" beside items measured an hour ago, and nothing in the format distinguishes them — so the next session reads an inherited assumption and a fresh measurement with equal confidence.

The failure is quiet in both directions. An item may already be **done** — merged, closed, resolved by someone else — in which case the handoff sends the next session to redo settled work. Or it may be *stale about its own preconditions*: still open, but no longer runnable the way it is described, because something else that landed since changed what it depends on.

Most carried items are one command from settled: `gh pr view <n> --json state`, `gh issue view`, a tracker lookup, running the script the item says to run. Run the check before copying, and write what you found. Where it genuinely cannot be checked — no credential, the service is down, it depends on someone else's answer — say so in the item itself: *"carried from <prior handoff>, not re-verified today."* That single clause is what lets the next reader tell an inherited assumption from an observation, which is exactly the asymmetry the RESUME workflow's *Load the Handoff* step tells them to look for.

Failure mode this prevents: a carried list survives several handoffs untouched, each copy making it look more established, until a session finally acts on it and finds the work already merged, the state different from the one claimed, and the script it was told to run broken by a change two sessions back. Nothing errors, and the wasted effort is invisible until someone checks.
